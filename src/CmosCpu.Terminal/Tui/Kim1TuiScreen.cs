...
public sealed class Kim1TuiScreen : ITerminalScreen
{
    ...
    private void BuildKeypadAndCpu(Window top)
    {
        ...
        var cpuFrame = new FrameView { Title = "CPU", X = 30, Y = 4, Width = Dim.Fill(), Height = 8 };
    ...
    private void BuildStatusBar(Window top)
    {
        _statusView = new View { Text = $"Status: {TerminalGuiRenderer.FormatStatus(false, _machine.Cpu.IsHalted)}", X = 0, Y = Pos.AnchorEnd(2), Width = Dim.Fill() };
        _instructionsView = new View { Text = $"Instructions: {_session.TotalInstructionsExecuted}", X = 0, Y = Pos.AnchorEnd(1), Width = Dim.Fill() };
